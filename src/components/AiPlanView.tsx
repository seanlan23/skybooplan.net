...
function HotelsSection({
  ...
  const [loc, setLoc] = useState<"all" | "center" | "beach" | "airport">("all");

  const fetchHotels = useServerFn(searchHotels);
  const effectiveCheckOut = checkOut ?? checkIn;

  // Primary query: the day's actual city.
  ...
  // Hub fallback: only fire when the primary search returned zero hotels and a
  // distinct regional hub is available (e.g. Siquijor → Philippines / Cebu).
  // No mock data is ever shown — if both queries are empty we render a clean
  // empty state with a direct Booking.com search CTA. Selection logic lives in
  // `@/lib/hotelSelection` so it can be unit tested without React.
  ...
  void loc; void setLoc; // location filter retired with mock data
              ...
              return (
                ...
                >
                  <div className="relative h-36 w-full bg-slate-100">
                    ...
                    {h.reviews > 0 && (
                      <div className="absolute top-9 right-2 rounded-md bg-white/90 px-1.5 py-0.5 text-[10px] text-slate-700">
