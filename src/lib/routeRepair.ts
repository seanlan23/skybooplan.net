...
import { type PlanViolation } from "@/lib/planValidation";
...
export function logisticsRepairPrompt(
  ...
  const system = slo
      ? "Popravi SAMO navedene fizikalne kršitve. Ne zaklepaj koledarja mest. Ne izmišljuj hotelov in restavracij. Število koledarskih dni ostane isto."
      : "Fix ONLY the listed physics violations. Do not lock a city calendar. Do not invent hotels or restaurants. Keep the same number of calendar days.";
  const user = [
