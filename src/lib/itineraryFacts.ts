...
export function applyItineraryFacts(plan: AiTripPlan, language?: string): void {
  ...
  relocateMealsOutOfMorning(plan);
  rewriteLateCheckoutCopy(plan);
}

function clockHour(raw: string | undefined): number | null {
...
/** Farewell dinners do not belong in the morning slot. */
export function relocateMealsOutOfMorning(plan: AiTripPlan): number {
  let n = 0;
  for (const day of plan.days ?? []) {
    if (!day.activities?.morning?.length) continue;
    const evening = day.activities.evening ?? [];
    const eveningBusy = evening.some((a) => {
      const blob = `${a.name} ${a.description ?? ""}`;
      return /check-?out|letališč|airport|mednarodni|flight|transfer|prevoz na letališ/i.test(blob);
    });
    const destSlot = eveningBusy ? "afternoon" : "evening";
    const keep: Activity[] = [];
    const move: Activity[] = [];
    for (const a of day.activities.morning) {
      const blob = `${a.name} ${a.description ?? ""}`;
      const meal =
        /večerja|dinner|abendessen|poslovilna/i.test(blob) &&
        !/zajtrk|breakfast|brunch|kosilo|lunch/i.test(blob);
      if (meal) move.push(a);
      else keep.push(a);
    }
    if (!move.length) continue;
    day.activities.morning = keep;
    day.activities[destSlot] = [...(day.activities[destSlot] ?? []), ...move];
    n += move.length;
  }
  return n;
}

/** Check-out at 19:30 is not "zjutraj". */
export function rewriteLateCheckoutCopy(plan: AiTripPlan): number {
  let n = 0;
  for (const day of plan.days ?? []) {
    if (!day.activities) continue;
    for (const slot of SLOTS) {
      day.activities[slot] = (day.activities[slot] ?? []).map((a) => {
        const hour = clockHour(a.arrivalTime);
        const blob = `${a.name} ${a.description ?? ""}`;
        if (!/check-?out|odhod iz hotela/i.test(blob)) return a;
        if (hour == null || hour < 16) return a;
        if (!/zjutraj/i.test(a.description ?? "")) return a;
        n += 1;
        return {
          ...a,
          description: (a.description ?? "")
            .replace(/zjutraj zaključi check-out[^.!?\n]*[.!?]?/gi, "Check-out pred odhodom na letališče.")
            .replace(/\bzjutraj\b/gi, "")
            .replace(/\s{2,}/g, " ")
            .trim(),
        };
      });
