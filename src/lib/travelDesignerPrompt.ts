...
- DAY COUNT & DEPARTURE: the complete itinerary must EXACTLY match the inclusive calendar days between START_DATE and END_DATE. Day N (the final day) MUST ALWAYS be the departure day (hotel check-out, airport transfer, international return flight home — then landing at the home airport — or drive/train home on ground trips). Never add extra days. Never omit Day N. Never treat Day N as a full sightseeing day in a new city. Checkout / Grab / airport check-in clocks bind to the selected international departure — never reuse them on a same-day domestic hop. If the last night is not at the hub and the international board is morning/midday, sleep at the hub the night before. RED-EYE RETURN (board 00:00–05:59): check-out and airport transfer MUST sit in the evening of day N−1 (~22:30). Day N contains ONLY the overnight flight plus afternoon landing at home — FORBIDDEN morning check-out, destination taxi, or any destination departure/transfer on Day N.
...
- STRICT GENERATION & FORMATTING CONSTRAINTS: 100% target language; exactly N inclusive calendar days; Day 1 = flight arrival/start; Day N = return home (daytime: hotel check-out, airport transfer, return international flight; red-eye boarded N−1: only in-air + home landing — or drive/train home on ground trips); fully fleshed morning/afternoon/evening on sightseeing days; no placeholders or cut-off sentences; return strictly valid parseable JSON — no markdown code fences or conversational intro/outro.
...
${
  isGroundMainTransport(fields.mainTransport)
    ? ""
    : `RED-EYE RETURN (board 00:00–05:59): check-out and airport transfer MUST sit in the evening of day ${n != null ? n - 1 : "N-1"} (~22:30). Day ${nText} contains ONLY the overnight flight plus afternoon landing at home — FORBIDDEN morning check-out, destination taxi, or any destination departure/transfer on Day ${nText}.
...
Every single activity (morning, afternoon, evening) must have a fully completed description (minimum 25 words). NEVER output placeholders, unfinished titles, or sentences ending with '...' or cut off mid-word. All three slot keys are required. Before/after flights the slot describes the flight or wait — not a fake destination programme. Vsa navodila glede prepovedi izletov IN časovnic so interna pravila. NIKOLI ne izpisuj 'Ne delaj izleta na X…', 'Prtljago vzemi s seboj…' ali 'Na letališču si že od prejšnjega večera…' v opise. Piši naravno: 'Večerna odjava iz hotela in prevoz na letališče', 'Mednarodni nočni let proti domu'.`;
