...
- days[].city is the city of THAT day's main programme — where the traveller spends the bulk of the day (and usually the previous night's hotel). NEVER copy the arrival-airport city onto every day. On a morning/daytime hop INTO a new base (train/flight first, then new-city sights), city is the destination from that day on (Shinkansen Osaka→Tokyo ⇒ Tokyo on that day and the following stay days; Shinjuku / Ghibli / Ginza are Tokyo, not Osaka). On a late hop AFTER sightseeing, city stays the origin (Wat Pho / Chinatown on 28 Oct = Bangkok, not Chiang Mai). Consecutive full stay days keep the same city string. Forbidden: a one-day flicker to a previous hub with no transfer that day.
- days[].title is a unique day name (what happens that day) — never "Dan 1" / "Day 1".
- hotels[] / accommodations[] = one row per consecutive SLEEP stay { city, nights, from_date, to_date }. Nights are hotel nights, not labelled day.city: Osaka 2 nights 20–22 Sep and Tokyo 5 nights 22–27 Sep when the Shinkansen move is on 22 Sep (day.city from that day on = Tokyo). Bangkok 2 nights 26–28 Oct and Chiang Mai 3 nights 28–31 Oct when day 28 is still a Bangkok sightseeing day with an evening hop. Forbidden: a single gateway-city row covering the whole trip. If the traveller listed nights per city in wishes, hotels[] MUST match those counts exactly — no extra nights on the first base. Forbidden: a boat/flight day trip to an island/town that already has a multi-night stay (use local sights on the current base instead). Without an explicit stay plan: an entry/exit transit metropolis (Bangkok, Kuala Lumpur, Toronto, Tokyo…) gets at most 2–3 nights at the start and 1–2 at the return, and ≤30% of the trip in total; interior cultural/mountain bases and islands/parks get ≥3 nights.
