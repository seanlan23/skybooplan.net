...
- hotels[] / accommodations[] = one row per consecutive stay { city, nights } matching days[].city. Forbidden: a single gateway-city row covering the whole trip. If the traveller listed nights per city in wishes, hotels[] and days[].city MUST match those counts exactly — no extra nights on the first base. Forbidden: a boat/flight day trip to an island/town that already has a multi-night stay (use local sights on the current base instead).
