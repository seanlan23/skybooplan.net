...
export function motorhomePromptRules(slo: boolean): string {
  return slo
    ? [
        ...
        "PREPOVEDANO: generične aktivnosti 'Lokalna večerja', 'Kosilo na Rivi', 'Pavza v kavarni', 'Jutranji sprehod / kava pred ogledom', 'če imaš še energijo', 'brez hitenja takoj z letališča' kot dnevni filler.",
        "Med mesti samo VOŽNJA. Dnevna etapa 500–700 km (max 6–7 h z postanki). PREPOVEDANO 1500–2200 km v enem dnevu. Nikoli notranji let z RV. V mesto javni prevoz/P+R.",
        ...
        "Vsaka baza/kamp: NAJMANJ 2 noči (dnevni izleti iz kampa, npr. 2 noči Istra, 2 noči otok). 1 noč samo za čisti transfer ali zadnji hub. PREPOVEDANO menjati kamp vsakih 24 ur.",
        "Lastno vozilo (izhodišče = cilj): outbound in inbound = logičen krog ALI povratek z vmesnimi tranzitnimi nočitvami. Day N = samo zadnja zmerna etapa domov (≤5 h).",
    ...
    : [
        ...
        "FORBIDDEN: generic fillers like 'Local dinner', 'Lunch on the waterfront', 'Café break', 'Morning walk & coffee', 'if you still have energy', 'don't rush from the airport' as daily padding.",
        "Inter-city = DRIVING. Daily stage 500–700 km (max 6–7 h with stops). FORBIDDEN 1500–2200 km in one day. No domestic flights with RV.",
        ...
        "Each camp/base: at least 2 nights (day trips from that camp). 1 night only for a pure transfer or final hub. FORBIDDEN to change camp every 24 hours.",
        "Own vehicle (start = end): outbound and inbound form a loop OR the return inserts 1-night transit bases. Day N is only the last moderate hop home (≤5 h).",
