...
const EN_TO_SL: Pair[] = [
  ...
  [/^Hike to the summit of\s+(.+)$/i, "Pohod na vrh $1"],
  [/^Hike to\s+(.+)$/i, "Pohod na $1"],
  [/^(.+?)\s+Morning Game Drive$/i, "Jutranji ogled divjadi: $1"],
  [/^Morning Game Drive\b(.*)$/i, "Jutranji ogled divjadi$1"],
  [/^Afternoon Game Drive\b(.*)$/i, "Popoldanski ogled divjadi$1"],
  [/^Snorkeling (?:Trip )?(?:at|in|on)\s+(.+)$/i, "Snorkljanje: $1"],
  [/^Beach relaxation (?:at|in)\s+(.+)$/i, "Sprostitev na plaži: $1"],
